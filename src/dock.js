...
module.exports = class Dock {
  ...
  getInitialSize () {
    const items = this.paneContainer.getPaneItems()
    // The item may not have been activated yet. If that's the case, just use the first item.
    const activePaneItem = this.paneContainer.getActivePaneItem() || items[0]
    let initialSize
    if (activePaneItem) initialSize = getPreferredSize(activePaneItem, this.location)
    if (initialSize == null && items.length) initialSize = DEFAULT_INITIAL_SIZE
    return initialSize ? Math.max(MINIMUM_SIZE, initialSize) : null
  }
